...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : mer. 18 août 2021 à 16:22
...
--
-- Base de données : `modifbonbarquette`
--
...
(IN `Nom` VARCHAR(55), IN `Prenom` VARCHAR(55), IN `DateNaiss` DATE, IN `Telephone` VARCHAR(55), IN `Mail` VARCHAR(55), IN `Identif` VARCHAR(55), IN `Pass` VARCHAR(55))  NO SQL
BEGIN
DECLARE testIdentifiant int;
SET testIdentifiant = (SELECT COUNT(Identifiant) FROM clients WHERE Identifiant = Identif AND Email = Mail);
IF (testIdentifiant > 0)
THEN
signal sqlstate '45000' set message_text = 'identifiant existe déjà dans la table clients !';
ELSE
INSERT INTO clients( `Nom`, `Prenom`, `Telephone`, `Email`, `Identifiant`, `Password`) Values(Nom, Prenom, Telephone, Mail, Identif, MD5(Pass));
END IF;
END$$
...
DELIMITER $$
CREATE TRIGGER `Triggerz_aliments_historique` BEFORE DELETE ON `aliments` FOR EACH ROW INSERT INTO all_aliments_historique (`Type_Aliment`,`Nom`, `Description`, `Prix`) 
Values(Old.Id_Type_Aliment, Old.Nom, Old.Description, Old.Prix_Aliment)
$$
DELIMITER ;
...
(4, 'Lebeau', 'Emmanuel', '0692869586', 'a@a.fr', 'Oui', '24ace383b70d09954259c15cfeb9b2fd'),
(5, 'Delaterasse', 'Jean', '0692869586', 'aa@aa.fr', 'Non', '6679e65c932b2c5ad98080ee21a4f0d4');
...
INSERT INTO `commande` (`Id_Commande`, `Num_Commande`, `Id_Client`, `Id_Menu`, `Id_Aliment`, `Date_Achat`, `Prix`, `Date_Update`, `Etat`, `Quantitee`) VALUES
(148, 'dfg4df56', 3, 11, NULL, '2021-08-18 00:00:00', '17.90', '2021-08-18 00:00:00', 1, 0),
(149, 'dfd6h5g6hj', 3, 11, NULL, '2021-08-18 00:00:00', '17.90', '2021-08-18 00:00:00', 1, 0);
...
DELIMITER $$
CREATE TRIGGER `Triggerz_Notification` AFTER INSERT ON `commande` FOR EACH ROW INSERT INTO notification (`Type_Notification`, `Id_Commande`,`Id_Aliment`, `Id_Menu`) 
...
INSERT INTO `notification` (`Id_Notification`, `Type_Notification`, `Id_Commande`, `Id_Aliment`, `Id_Menu`) VALUES
(17, 1, 148, NULL, 11),
(18, 1, 149, NULL, 11);
